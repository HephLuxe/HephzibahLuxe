...
Daily digest email for an upcoming payment milestone
(`document_hub.tasks.payment_due_digest_task`).

| | |
|---|---|
| `template_name` | `payment_due` |
| Env var | `BREVO_TEMPLATE_PAYMENT_DUE` |
| Fired by | `document_hub.tasks.payment_due_digest_task` (daily beat) |
| Merge fields | `label`, `amount`, `due_date`, `event_title` |

**Suggested subject:** `Payment reminder: {{ params.label }}`
**Preheader:** `A payment on your Hephzibah Luxe schedule is coming due soon.`
...
2. Replace `PORTAL_BASE_URL_HERE` with your deployed frontend root (same
   value as `settings.FRONTEND_BASE_URL`) — the CTA points at
   `/portal/financials/client`, the real route for `invoice`/`receipt`/
   `payment_milestone` targets (`apps/core/deeplinks.py`).
3. Paste the subject, save, set `BREVO_TEMPLATE_PAYMENT_DUE=<id>`.
...
<body style="margin:0;padding:0;background-color:#F3F1EC;">
  ...
  <div style="display:none;max-height:0;overflow:hidden;mso-hide:all;font-size:1px;line-height:1px;color:#F3F1EC;opacity:0;">
    A payment on your Hephzibah Luxe schedule is coming due soon.
    &#8199;&#65279;&#8199;&#65279;&#8199;&#65279;&#8199;&#65279;&#8199;&#65279;&#8199;&#65279;&#8199;&#65279;
  </div>
        ...
        <table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background-color:#FFFFFF;border-radius:16px;overflow:hidden;border:1px solid #ECEAE4;">
          ...
          <tr>
            <td class="px" style="padding:46px 48px 0;">
              <div style="font-family:'Inter',Arial,sans-serif;font-size:12px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:#B79A66;">Payment reminder</div>
              <h1 class="h1" style="margin:12px 0 0;font-family:'Cormorant Garamond',Georgia,serif;font-size:36px;line-height:1.15;font-weight:600;color:#062025;">{{ params.label }} is due soon</h1>
            </td>
          </tr>
          ...
          <tr>
            <td class="px" style="padding:22px 48px 0;font-family:'Inter',Arial,sans-serif;font-size:16px;line-height:1.7;color:#4A5456;">
              A payment on your <strong>{{ params.event_title }}</strong> planning schedule is coming due. Here are the details:
            </td>
          </tr>

          <!-- detail card: amount | due date -->
          <tr>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#F8F7F4;border:1px solid #ECEAE4;border-radius:12px;">
                <tr>
                  ...
                  <td class="col2" width="50%" valign="top" style="padding:22px 28px;font-family:'Inter',Arial,sans-serif;">
                    ...
                    <div style="margin-top:6px;font-size:16px;color:#062025;font-weight:500;">{{ params.due_date }}</div>
                  </td>
                </tr>
              </table>
            </td>
...
- **`amount`** arrives as a bare numeric string (`Decimal` → `str` via
  `notifications.services._serialise_value`) — the template prefixes `₦`
  itself. If you ever need thousands separators, format the number before it
  enters `context` at the call site (`document_hub.tasks.payment_due_digest_task`),
  not in Brevo.
- **`due_date`** is already an ISO string (`YYYY-MM-DD`) by the time it
  reaches Brevo. For a friendlier "31 July 2026" style, format it at the call
  site with `date.strftime(...)` before passing it to `queue_notification`.
- The CTA is a **static route**, not a per-record deep link (this digest has
  no `link_url` param) — it goes to the financials tab generally, which is the
  same route `apps/core/deeplinks.py` uses for `payment_milestone` targets.
